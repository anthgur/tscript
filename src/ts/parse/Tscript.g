...
assignmentExpression
  ...
  | l=leftHandSideExpression EQUAL r=assignmentExpression
    { checkAssignmentDestination(loc($start), $l.lval);
      $lval = buildBinaryOperator(loc($start), BinOpcode.ASSIGN,
        $l.lval, $r.lval); }
  ;
...
unaryExpression
  ...
  | EXCLAMATION r=unaryExpression
    { $lval = buildUnaryOperator(loc($start), UnOpcode.NOT, $r.lval); }
  | PLUS r=unaryExpression
    { $lval = buildUnaryOperator(loc($start), UnOpcode.PLUS, $r.lval); }
  | MINUS r=unaryExpression
      { $lval = buildUnaryOperator(loc($start), UnOpcode.MINUS, $r.lval); }
  ;
...
additiveExpression
  ...
  | l=additiveExpression PLUS r=multiplicativeExpression
    { $lval = buildBinaryOperator(loc($start), BinOpcode.ADD,
        $l.lval, $r.lval); }
  ;
...
multiplicativeExpression
  ...
  | l=multiplicativeExpression ASTERISK r=unaryExpression
    { $lval = buildBinaryOperator(loc($start), BinOpcode.MULTIPLY,
      $l.lval, $r.lval); }
  ;
